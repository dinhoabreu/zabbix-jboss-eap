...
class JMXItemChecker extends ItemChecker
{
	...
	private JMXServiceURL url1;
	private JMXServiceURL url2;
	...
	public JMXItemChecker(JSONObject request) throws ZabbixException
	{
		...
		try
		{
			...
			urlCache = new File(System.getProperty("java.io.tmpdir") +
				"/zabbix_java_gateway_" + conn + "_" + port);
			url1 = new JMXServiceURL("service:jmx:rmi:///jndi/rmi://[" + conn + "]:" + port + "/jmxrmi");
			url2= new JMXServiceURL("service:jmx:remoting-jmx://" + conn + ":" + port);
			jmxc = null;
			mbsc = null;
	...
	public JSONArray getValues() throws ZabbixException
	{
		...
		try
		{
			...
			if (urlCache.exists()) {
				logger.info("Hit cache file " + urlCache.getAbsolutePath());
				jmxc = ZabbixJMXConnectorFactory.connect(url2, env);
				mbsc = jmxc.getMBeanServerConnection();
			} else {
				try
				{
					jmxc = ZabbixJMXConnectorFactory.connect(url1, env);
					mbsc = jmxc.getMBeanServerConnection();
				}
				catch (Throwable e)
				{
					logger.info(e.getMessage() + " fallback to " + url2);
					jmxc = ZabbixJMXConnectorFactory.connect(url2, env);
					mbsc = jmxc.getMBeanServerConnection();
					logger.info("Create cache file: " + urlCache.getAbsolutePath());
					urlCache.createNewFile();
				}
			}
			for (String key : keys)
